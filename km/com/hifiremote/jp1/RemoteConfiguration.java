...
//        Collections.sort( macros, MacroSorter );
  ...
  public void loadSegments( boolean decode )
  {
    ...
    if ( upgradeList != null )
    {
      for ( Segment segment : upgradeList )
      {
        ...
        if ( protocolOffset > 0 && protocolOffset < hex.length() - 4 )
        {
          ...
          {
            // This is a protection against a situation that has been seen generated by missing
            // commas in the [Protocols] section.  No valid protocol has a length < 4.  This case
            // should not arise, other than through such errors.
            protocolCode = null;
          }
          else if ( remote.usesEZRC() || ( protocolCode.getData()[ 3 ] & 0x40 ) > 0 )
          {
    ...
    if ( activityAssignments == null  && remote.getSegmentTypes().contains( 0xCD ) 
        && remote.hasActivityControl() )
    {
      for ( int i= 0; i < remote.getActivityControl().length; i++ )
      {
        List< Segment > list = segments.get( 0xCD + i );
    ...
    {      
      ...
      for ( int i = 0; i < activityBtns.size(); i++ )
      {
        ...
        int index = hex.getData()[ indexPos ];
        ...
        {
          String selectorName = "" + ( index + offset );
          activity.setSelector( remote.getButton( selectorName ) );
        }
        index &= 0x7F;
        if ( index < control.maps.length )
        {     
          int val = control.maps[ index + offset - 1 ];       
          DeviceButton[] devBtns = remote.getDeviceButtons();
...
//            f = new Function( text, cmd, null );
  ...
  public int getDefaultActivitySetting( int index)
  {
    ...
    for ( int i = 1; i >= 0; i-- )
    {
      int map = remote.getActivityControl()[ index ].maps[ i ];
      if ( ( map & states ) == map )
      {
        val = 0x81 + i;
        break;
