...
//    public String[] comments = new String[ 2 ];
  ...
  public String analyze( Processor p, Hex hex )
  {
    ...
    simple = true;
    String name = e != null && e.names.size() > 0 ? e.names.get( 0 ) : "protocol";
    proc = p;
    procNative = ProcessorManager.getProcessor( p.getNativeProcessorName() );
    if ( procNative != null )
    {
      procNative.setAddressList( new ArrayList< Integer >() );
    }
    labels.clear();
    ...
    int pos = proc.getProtocolHeaderSize(); // skip header
    sb.append( "Header:\n" );
    sb.append( addrHex( 0 ) + hex.subHex( 0, pos ) + "\n" );
    ...
    int dbStart = pos++;
    int dbHeader = data[ dbStart ];
    int dbSize = dbHeader & 0x07;
    if ( dbSize == 1 || dbSize > 3 )
    {
    ...
    int dbSwitchIndex = ( dbSwitch >> 12 ) & 0x0F;
    int dbSwitchSize = ( dbSwitch >> 8 ) & 0x0F;
    int dbSwitchMaskStart = 0;
    
    if ( dbSwitchSize > 0 )
    {
      int[] rev = new int[ 4 ];
      reverseByte( dbSwitchMask, rev );
    ...
    int dbOffset = dbSize > 2 ? data[ dbStart + 3 ] : 0;
    boolean hasAltExec = ( dbHeader & 0x20 ) != 0 && dbOffset > 0;
    if ( hasAltExec )
    {
    ...
    tbUsed = 0xFF;  // Interpret all entries as having default meaning
    initialCodeSpec = 0;
    pos = tbStart;
    sb.append( addrHex( pos ) + hex.subHex( pos, 1 ) + " / " );
    sb.append( ( hex.subHex( ++pos, tbSpecSize ) + " / " ) );
    sb.append( hex.subHex( pos += tbSpecSize, tbDataSize ) + "\n" );
    for ( String str : analyzeTimingBlock( hex.subHex( tbStart, 1 + tbSize ), true, 0, 0x40, null ) )
    {
      sb.append( str + "\n" );
    }
    sb.append( "\n" );
    ...
    sb.append( "OFF times are in units of " + ( has2usOff ? "2us\n" : "carrier cycles\n" ) );
    if ( hasNativeCode )
    {
    ...
    if ( hasAltExec )
    {
      sb.append( "*** Protocol block is followed by an alternate executor at " + String.format( "$%04X.  ", pbStart + dbOffset )
          + "This option has never been seen and its operation is unclear.  "
          + "Parsing here covers only the initial executor.\n" );
    }
    sb.append( "\n" );
    
    String pbSelector = "(" + getZeroLabel( proc.getDcBufStart() + dbSwitchIndex ) + " & " + String.format( "$%02X) = ", dbSwitchMask );
    ...
    do
    {
      ...
      pbOffset = pbOptSize > 0 ? data[ pos + 1 ] : 0;
      ...
      int codeSpec = pbOptSize > 3 ? data[ pos + 4 ] : 0;
      initialCodeSpec = codeSpec;
      int toggle = pbOptSize > 5 ? hex.get( pos + 5 ) : 0;
      pos += 1 + pbOptSize;
      int pbCodeSize = pbHasCode ? data[ pos ] : 0;
      if ( pbHasCode )
      {
        ...
        disassemblePseudocode( addr, hex.subHex( addr, pbCodeSize ), "", flags, 1 );
        pos += pbCodeSize;
      }
      int sbStart = pos;
      ...
      do
      {
        ...
        sigBlkCount++;
        int sbHeader = data[ pos ];
        int formatLen = sbHeader & 0x07;
        
        if ( sigBlkCount == 1 )
        {
          // Set pf[] to that for the first signal block, which is that
          // read before protocol code run
          pf = new int[ 7 ];
        ...
        int pf2 = formatLen > 1 ? data[ pos + 2 ] : 0;
        int pf5 = formatLen > 4 ? data[ pos + 5 ] : 0;
        int sigSpecLen = pf2 & 0x1F;
        int rptType = pf2 >> 5;
        mayExecNextBlock = rptType == 1 || rptType == 2 || rptType == 4;
        pos += 1 + formatLen + sigSpecLen;
        if ( pos + ( sbHasCode ? 1 : 0 ) > protLen )
        {
          return endErr;
        } 
        int sbCodeSize = sbHasCode ? data[ pos ] : 0;
        if ( sbHasCode )
        {
          ...
          int addr = ++pos;
          boolean[] flags = new boolean[ 20 ];
          disassemblePseudocode( addr, hex.subHex( addr, sbCodeSize ), "", flags, 1 );
          pos += sbCodeSize;
        }
        hasKeyDepSignalBlock = ( pf5 & 0xF0 ) != 0;
        if ( pbSwitchSize > 0 && ( hasKeyDepSignalBlock || sbHasCode || hasNativeCode || mayExecNextBlock ) 
            || hasKeyDepSignalBlock && ( sbHasCode || hasNativeCode || mayExecNextBlock )
            || hasNativeCode && mayExecNextBlock )
        {
        ...
        {
          // These are alternate signal specs
          if ( pos >= protLen )
          {
            return endErr;
          }
        ...
        if ( !hasNativeCode && !mayExecNextBlock && ( pbOffset > 0 && pbOffset > pos - pbStart 
            || pbOffset == 0 && pos < protLen ) )
        {
          // Assume another signal block required by code
      ...
      sb.append( dbSwitchSize > 0 ? " when " + pbSelector
          + String.format( "$%02X", pbIndex << dbSwitchMaskStart) + ":" 
          : ":" );
      sb.append( "\n" );
      pos = pbStart;
      sb.append( addrHex( pos ) + hex.subHex( pos++, 1 ) + " / " );
      ...
      List< String > list = interpretCodeSelector( codeSpec );
      if ( !list.isEmpty() )
      {
      ...
      if ( pbHasCode )
      {
        ...
        int addr = pos + 1;
        boolean[] flags = new boolean[ 20 ];
        sb.append( disassemblePseudocode( addr, hex.subHex( addr, pbCodeSize ), "", flags, 2 ) );
      }
      int[] togData = null;
      if ( toggle > 0 )
      {
        ...
        sb.append( analyzeToggle( toggle, togData ) );
      }
      sb.append( "\n" );
      ...
      do
      {
        sbIndex++;
        sbStart = pos++;
        int sbHeader = data[ sbStart ];
        int formatLen = sbHeader & 0x07;
        int pf2 = formatLen > 1 ? data[ sbStart + 2 ] : 0;
        int pf5 = formatLen > 4 ? data[ sbStart + 5 ] : 0;
        int pf6 = formatLen > 5 ? data[ sbStart + 6 ] : 0;
        ...
        mayExecNextBlock = rptType == 1 || rptType == 2 || rptType == 4;
        pos += formatLen;
        Hex signalSpec = hex.subHex( pos, sigSpecLen );
        pos += sigSpecLen;
        boolean sbHasCode = ( sbHeader & 0x80 ) != 0;
        hasKeyDepSignalBlock = ( pf5 & 0xF0 ) != 0;
        int sbCodeSize = sbHasCode ? data[ pos ] : 0;
        ...
        String sbIntro = keyCondition.isEmpty() && sigBlkCount > 1 ?
            ( new String[]{ "First", "Second", "Third", "Fourth", "Next" } )[ sbIndex > 4 ? 4 : sbIndex ] + " signal" : "Signal";
        
        sb.append( sbIntro + " block (PF bytes / signal spec / code size)" + keyCondition + ":\n" );
        sb.append( addrHex( sbStart ) + hex.subHex( sbStart, 1 + formatLen ) + " / " );
        ...
        int[][] pfDescs = new int[][]{ {-1,1,6}, {0x80,4,0 }, {0x08,1,3}, {0x04,1,2},
            {-1,0,6}, {0x3F,3,0}, {-1,1,4}, {0xE0,2,5}, {0x80,3,7}, {0xF0,5,0}, {0x80,6,0} };
        for ( int[] pfDesc : pfDescs )
        {
          if ( pfDesc[ 0 ] < 0 || ( pf[ pfDesc[ 1 ] ] & pfDesc[ 0 ]) > 0 )
          {
            sb.append( getPFdescription( pfDesc[ 1 ] , pfDesc[ 2 ], null ) + "\n" );
          }
        }
        sb.append( "\n" );
        
        sb.append( interpretSignalSpec( signalSpec ) + "\n" );

        String altSelector = "(" + getZeroLabel( proc.getDcBufStart() + pbSwitchIndex ) + " & " + String.format( "$%02X) = ", pbSwitchMask );
        if ( pbSwitchSize > 0 )
        {
          // Note that when this is so, there is no code block or further signal blocks
        ...
        for ( int i = 0; i < pbSwitchSize; i++ )
        {
          ...
          sb.append( " when " + altSelector + String.format( "$%02X\n", ( i + 1 ) << pbSwitchMaskStart) );
          sb.append( interpretSignalSpec( altSignalSpec ) + "\n" );
          pos += altSigSpecLen;
        }
        if ( pbSwitchSize > 0 )
        {
        ...
        if ( sbHasCode )
        {
          sb.append( "Signal block code (run "
              + ( sbCodeBeforeTX ? "before" : "after" )
              + " signal spec translation of signal data):\n" );
          int addr = pos + 1;
          boolean[] flags = new boolean[ 20 ];
          sb.append( disassemblePseudocode( addr, hex.subHex( addr, sbCodeSize ), "", flags, 2 ) );
          sb.append( "\n" );
        }
        ...
        if ( hasNativeCode )
        {
          ...
          sb.append( disassembleNativeCode( pos, hex.subHex( pos, protLen - pos ), "" ) );
          sb.append( "\n" );
          pos = protLen;
        }
        if ( !hasNativeCode && !mayExecNextBlock && !hasKeyDepSignalBlock 
            && ( pbOffset > 0 && pbOffset > pos - pbStart 
            || pbOffset == 0 && pos < protLen ) )
        {
          // Assume another signal block required by code
      ...
      if ( pbOffset > 0 && pbOffset < pos - pbStart )
      {
  ...
  private String interpretSignalSpec( Hex txBytes )
  {
    ...
    txStr += translateTXBytes( txBytes, temp );
    return txStr;
  }
  ...
  private List< String > interpretCodeSelector( int codeSpec )
  {
    ...
    if ( ( codeSpec & 0x10 )> 0 )
    {
      ...
      for ( int i = 0; i < 7; i++ )
      {
        n += tbLengths[ i ] * sizes[ i ];
      }
      if ( n + tbLengths[ 7 ] - 1 < tbDurations.length )
      {
        Hex hex = new Hex( Math.max( 2 * tbLengths[ 7 ], proc.getProtocolHeaderSize() ) );
        for ( int j = 0; j < 2 * tbLengths[ 7 ]; j++ )
        {
          int val = tbDurations[ n + ( j >> 1 ) ];
          int k = 8 * ( j & 1 );
          hex.set( ( short )( ( val >> k ) & 0xFF ), j );
        }
        int[] carrierData = proc.getCarrierData( hex );      
        if ( carrierData[ 0 ] > 0 )
        {
          list.add( "The MARK of a 0-burst is sent with the following alternate carrier:" );
          list.add( String.format( "  %.2fkHz, duty cycle %.1f", proc.getOscillatorFreq() / ( 1000.0 * carrierData[ 0 ] ), carrierData[ 1 ] / 10.0 ) + "%" );
          altCarrier = carrierData[ 0 ];
        }
      }
    }
    if ( ( codeSpec & 0x20 ) != 0 )
    ...
    if ( codeSelector == 1 )
    {
      ...
      for ( int i = 0; i < 4; i++ )
      {
        ...
        {
          int val = durationToMicrosecs( tbDurations[ n++ ], carrier, true );
          if ( val > 0 )
          {
            str += str.isEmpty() ? "" : ",";
            str += "+" + val;
          ...
          val = durationToMicrosecs( tbDurations[ n++ ], carrier, !has2usOff );  // convert carrier cycles to us
          if ( val > 0 )
          {
            str += str.isEmpty() ? "" : ",";
            str += "-" + val;
...
//      }
  ...
  private String disassemblePseudocode( int addr, Hex hex, String prefix, boolean[] flags, int returnMode )
  {
    ...
    if ( returnMode == 1 )
    {
      ...
      {
        AssemblerItem item = itemList.get( i );
        String hexOut = item.getOpCode().getName().equals( "END" ) ? "\t           " : addrHex( item.getAddress() ) + item.getHex().toRawString();
    ...
    {    
      ...
      for ( int k = i; k < j; k++ )
      {
        ...
        str += item.getLabel() + "\t" + item.getOpCode().getName() + "\t" + argText;
  ...
  {
//    proc = procNative != null ? procNative : ProcessorManager.getProcessor( "MAXQNative" );
    String codeStr = "";
    if ( procNative == null )
    {
      ...
      while ( remain > 0 )
      {
        int len = remain > 0x10 ? 0x10 : remain;
        codeStr += addrHex( addr ) + hex.subHex( index, len ).toString() + "\n";
        index += len;
        remain -= len;
        addr += len;
      }
    ...
    proc = procNative;
    proc.setRelativeToOpStart( true );
    proc.setOneByteAbsoluteAddresses( false );
    proc.getAddressList().clear();
    if ( hex == null )
    {
      codeStr += "*** Code block invalid\n";
      return codeStr;
    ...
    while ( index < pHex.length() )
    {
      ...
      AssemblerOpCode oc  = proc.getOpCode( pHex.subHex( index ) );
      AddressMode mode = oc.getMode();
      int incr = oc.getLength() + mode.length;
      ...
      for ( int i = 0; ( mode.relMap >> i ) != 0; i++ )
      {
        if ( ( ( mode.relMap >> i ) & 1 ) == 1 )
        {
          ...
          if ( n < data.length )
          {
            int newAddr = addr + index + 2 * ( data[ n ] - ( data[ n ] > 0x7F ? 0x100 : 0 ) ) + incr;
            if ( !labelAddresses.contains( newAddr ) )
            {
              labelAddresses.add( newAddr );
            }
    ...
    for ( int i = 0; i < itemList.size(); i++ )
    {
      ...
      String str = addrHex( item.getAddress() ) + hexOut + "\t" + item.getLabel() + "\t";

      if ( !proc.getAddressList().contains( item.getAddress() ) )
      {
        str += item.getOpCode().getName() + "\t" + item.getArgumentText();
      }
      codeStr += str + "\n";
    }
    return codeStr;
...
//    }
    ...
    String irpName = altCount > 0 ? irpLabel( proc.getDcBufStart() + altIndex ) : "";
  ...
  public String analyzeHeader( Hex hex )
  {
    ...
    {
      double freqKhz = proc.getOscillatorFreq() / 1000.0;
      double duty = carrierData[ 1 ] / 10.0;
      s += String.format( "%.2fkHz, duty cycle %.1f", freqKhz / carrier, duty ) + "%\n";
      irpStruct.generalSpec = String.format( "{%.1fk,", freqKhz / carrier );
    }
    int args = hex.getData()[ proc.getProtocolHeaderSize() - 1 ];
    fix = ( args >> 4 ) & 0x0F;
    var = args & 0x0F;
    s += "" + fix + " fixed, " + var + " variable bytes";
    List< String[] > allZeroLabels = new ArrayList< String[] >();
    int start = proc.getDcBufStart();
    if ( fix > 0 )
    {
    ...
    allZeroLabels.addAll( Arrays.asList( proc.getBaseZeroLabels() ) );
    proc.setZeroLabels( allZeroLabels.toArray( new String[ 0 ][ 0 ]) ); 
    if ( simple )
    {
      if ( procNative != null )
      {
        procNative.setZeroLabels( allZeroLabels.toArray( new String[ 0 ][ 0 ]) );
      }
      if ( fix > 0 || var > 0 )
      {
        s += " (Read into ";
      ...
      if( fix > 0 )
      {
        s += "Fix0" + ( fix > 1 ? "-Fix" + ( fix - 1 ) : "" ) + ( var > 0 ? ", " : "" );       
      }
      if ( var > 0 )
      {
        s += "Var0" + ( var > 1 ? "-Var" + ( var - 1 ) : "" );
      }
      if ( fix > 0 || var > 0 )
      {
        s += ")";
  ...
  private int calculateUnit( int provUnit, int tolerance )
  {
    ...
    for ( int i = 0, n = 0; i < 7; i++ )
    {
      ...
      {
        int val = durationToMicrosecs( tbDurations[ n ], carrier, ( j & 1 ) == 0 && sizes[ i ] == 2 || !has2usOff );
        usDurations[ n++ ] = val;
      }
    }
...
//      }
  ...
  private String getTimingItem( int n, int itemCarrier, int[] durations, int[] limits, List< Integer > irpVals, List< String > warn )
  {
    ...
    for ( int i = 0; i < tbLengths[ n ]; i++ )
    {
      if ( sizes[ n ] == 2 )
      {
        int val = durationToMicrosecs( durations[ pos++ ], itemCarrier, true );
        if ( val > 0 )
        {
          str += str.isEmpty() ? "" : ",";
          str += "+" + val;
        ...
        val = durationToMicrosecs( durations[ pos++ ], itemCarrier, !has2usOff );//( durations[ pos++ ]*mult + 3 )/6;  // convert carrier cycles to us
        if ( val > 0 )
        {
          str += str.isEmpty() ? "" : ",";
          str += "-" + val;
      ...
      else
      {
        ...
        if ( n != 7 )
        {
          val = durationToMicrosecs( val, itemCarrier, !has2usOff );
          if ( val > 0 )
          {
            str += "-" + val;
            if ( irpVals != null )
  ...
  private String analyzeToggle( int toggle, int[] togData )
  {
    ...
    String label = getZeroLabel( proc.getDcBufStart() + index );
    if ( ( type & 0x04 ) != 0 )
    {
 ...
 */
  private String analyzeProtocolBlock( int addr, Hex hex, LinkedList< TimingStruct > altTimings )
  {
    ...
    else
    {
      ...
      int cbSize = data[ pos++ ];
      s += disassemblePseudocode( addr + pos, hex.subHex( pos, cbSize ), "", flags, 0 );
...
//      }
    ...
    if ( pbSwitchSize > 0 )
    {
      ...
      String irpName = irpLabel( proc.getDcBufStart() + pbSwitchIndex );
    ...
    while ( more )
    {
      ...
      if ( sbHasCode )
      {
        ...
        codeStr += ":\n";
        int cbSize = data[ pos++ ];
        codeStr += disassemblePseudocode( addr + pos, hex.subHex( pos, cbSize ), "  ", flags, 0 );
  ...
  private int[] interpretToggle( int toggle )
  {
    ...
    if ( toggle > 0 )
    {
      ...
      {
        String label = getZeroLabel( proc.getDcBufStart() + togData[ 4 ] );
    ...
    else if ( pfn == 5 )
    {
      ...
      for ( int i = 0; i < 4; i++ )
      {
        if ( ( val & mask ) > 0 )
        {
          ...
          desc += " then use this Signal Block, otherwise use the next one";
          break;
        }
        mask <<= 1;
      }
...
//      }
    ...
    String str = "";
    str = opIndex == 3 && args.contains( "StatFlags, #$01" ) ? args.contains( "NZ" ) 
        ? "Branch if not first frame" : "Branch if first frame"
        : opIndex == 1 && args.equals( "StatFlags, StatFlags, #$FE" ) ? "Reset to no frames sent"
        : opIndex == 2 && args.equals( "CtrlFlags, CtrlFlags, #$01" ) ? "Suppress IR transmission"
        : opIndex == 1 && args.equals( "CtrlFlags, CtrlFlags, #$FE" ) ? "Resume IR transmission" : "";
    ...
    if ( opIndex == 6 || opIndex == 7 )
    {
      itemType = 3;
      Hex hex = new Hex( 5 );
      int start = ( proc.getProtocolHeaderSize() > 3 ) ? 1 : 0;
      ...
      if ( opIndex == 7 )
      {
        carriers[ 1 ] = has2usOff ? 12 : carriers[ 0 ];
        ts.carriers[ 1 ] = carriers[ 1 ];
      }
      str = String.format( "Set %.2fkHz, duty cycle %.1f", proc.getOscillatorFreq() / ( 1000.0 * carrierData[ 0 ] ), carrierData[ 1 ] / 10.0 ) + "%";
        ...
        || opIndex == 7 )
    {
      ...
      double f = proc.getOscillatorFreq() / ( 1000.0 * carrier );
      ...
      { 
        int duration = durationToMicrosecs( durations[ dest ], carrier, true );
  ...
  {
    double mult = mark ? itemCarrier * ( 1000000.0 / proc.getOscillatorFreq() ) : 2.0;
    return ( int )( duration * mult + 0.500001 );
  }
  ...
  private String translateTXBytes( Hex hex, List< String > srcList )
  {
    ...
    for ( int i = 0; i < txdCount; i++ )
    {
      ...
      txdIndex[ i ] = val & 0x0F;
      int addr = proc.getDcBufStart() + txdIndex[ i ];
      ...
      LinkedHashMap< String, OpTree > incrementals )
  {
    ...
    while ( st.hasMoreTokens() )
    {
      ...
      if ( argNum == 0 )
      {
        ...
        if ( ref )
        {
          dest = irpLabel( proc.getDcBufStart() ) + "[" + dest+ "-208]";
        }
      }
      else if ( argNum == 1 )
      {
...
//        }
    ...
    if ( op == 0x4C )
    {
      str = irpLabel( proc.getDcBufStart() ) + "[" + irpLabel( hex[ 1 ] ) + "]";
      if ( !destList.contains( str ) )
      {
        destList.add( str );
      }
